  ...
  return (
    <div className="fixed inset-0 z-[100] flex items-center justify-center bg-black/60 backdrop-blur-md p-4 animate-in fade-in duration-300">
      ...
      <div className="bg-[var(--card-bg)]
        ...
        <button
          onClick={onClose}
          className="absolute top-6 right-6 text-2xl opacity-40 hover:opacity-100 transition-opacity hover:rotate-90 duration-300"
        >
          ✕
        </button>
        ...
        <div className="grid grid-cols-1 md:grid-cols-3 gap-4 mb-8">
          ...
          <div className="bg-[var(--bg-secondary)]/50 border border-[var(--accent)]/10 rounded-2xl p-4">
            ...
            <p className="text-sm font-black tracking-tight">{data.spotifyReset}</p>
          </div>
          <div className="bg-[var(--bg-secondary)]/50 border border-[var(--accent)]/10 rounded-2xl p-4">
            <p className="text-[10px] uppercase font-bold text-[var(--text-primary)] opacity-60 mb-1">Apple Music Reset</p>
            <p className="text-sm font-black tracking-tight">{data.appleReset}</p>
          </div>
        </div>
        ...
        <div className="flex items-center justify-between mb-4 px-1">
          ...
          <div className="flex bg-[var(--bg-secondary)] p-1 rounded-xl border border-[var(--accent)]/10">
            <button
        ...
        <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-3 mb-8">
          {currentPlaylists.map((pl, idx) => (
            <a
              ...
              className="group bg-[var(--bg-secondary)]/40 hover:bg-[var(--accent)] border border-[var(--accent)]/10 hover:border-transparent 
                         rounded-xl p-3.5 flex items-center justify-between transition-all duration-300"
            >
              <span className="text-[11px] font-bold group-hover:text-black dark:group-hover:text-black transition-colors">{pl.name}</span>
